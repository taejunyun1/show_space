...
export function alignRasterCorners(input:WallCandidate[]):WallCandidate[]{
 const proposals:{a:number;ae:'start'|'end';b:number;be:'start'|'end';thin:boolean;point:{x:number;y:number}}[]=[];
 for(let i=0;i<input.length;i++)for(let j=i+1;j<input.length;j++){
  ...
  const thin=a.thicknessPx<3||b.thicknessPx<3;
  ...
  const trimTolerance=(line:WallCandidate,base:number,endpoint:'start'|'end')=>{
   ...
   if(thin)return t>=0&&t<=1&&(endpoint==='start'?t<=.5:t>=.5)?4:-1;
  ...
  for(const ae of ['start','end'] as const)for(const be of ['start','end'] as const){
   if(Math.hypot(a[ae].x-point.x,a[ae].y-point.y)<=trimTolerance(a,aTolerance,ae)&&Math.hypot(b[be].x-point.x,b[be].y-point.y)<=trimTolerance(b,bTolerance,be))proposals.push({a:i,ae,b:j,be,point,thin});
  }
 }
 // Thin annotation strokes must not veto an already observed thick corner.
 const thickEndpoints=new Set(proposals.filter(p=>!p.thin).flatMap(p=>[`${p.a}:${p.ae}`,`${p.b}:${p.be}`]));
 const accepted=proposals.filter(p=>!p.thin||(!thickEndpoints.has(`${p.a}:${p.ae}`)&&!thickEndpoints.has(`${p.b}:${p.be}`)));
 const targets=new Map<string,Set<string>>(),key=(p:{x:number;y:number})=>`${p.x},${p.y}`;
 for(const p of accepted)for(const endpoint of [`${p.a}:${p.ae}`,`${p.b}:${p.be}`]){const values=targets.get(endpoint)??new Set();values.add(key(p.point));targets.set(endpoint,values);}
 const result=input.map(l=>({...l,start:{...l.start},end:{...l.end}}));
 for(const p of accepted){if(targets.get(`${p.a}:${p.ae}`)!.size!==1||targets.get(`${p.b}:${p.be}`)!.size!==1)continue;result[p.a][p.ae]={...p.point};result[p.b][p.be]={...p.point};}

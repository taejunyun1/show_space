import {applyOverlaidDoors,activeOverlaidDoors,type OverlaidDoor} from './overlaidDoors';
...
export function resolvePlanOpenings(walls:Wall[],labels:PlanLabel[],maxGap:number,stairs:StairRegion[]=[],lines:WallCandidate[]=[],doors:OverlaidDoor[]=[]){
 const overlay=applyOverlaidDoors(walls,activeOverlaidDoors(doors,labels));walls=overlay.walls;
 labels=labels.filter(l=>!l.id.startsWith('overlaid-door:'));
